...
class User
{
    ...
    public static function getById($uid)
    {
        ...
        $query = sprintf('SELECT USERNAME, PASSWORD, EMAIL_ADDR, IS_ACTIVE, USER_PRIV ' .
    ...
    public static function getByUsername($username)
    {
        ...
        $query = sprintf('SELECT USER_ID, PASSWORD, EMAIL_ADDR, IS_ACTIVE, USER_PRIV ' .
	...
	// return score
	public static function getScoreByUsername($username)
	{
		$u = new User();

        $query = sprintf('SELECT USER_ID, IS_ACTIVE ' .
            'FROM %sUSER WHERE USERNAME = "%s"',
            DB_TBL_PREFIX,
            mysql_real_escape_string($username, $GLOBALS['DB']));
        $result = mysql_query($query, $GLOBALS['DB']);
        ...
        mysql_free_result($result);
        if($u->isActive == 1){
        	$query = sprintf('SELECT USER_ID, SCORE1, SCORE2, SCORE3, SCORE4, CASH1, CASH2, CASH3, CASH4 ' .
            'FROM %sINVEST WHERE USER_ID = "%d"',
            DB_TBL_PREFIX,
            $u->uid);
        	$result = mysql_query($query, $GLOBALS['DB']);
        	if (mysql_num_rows($result))
